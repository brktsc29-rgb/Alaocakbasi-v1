          ...
          <div className="max-w-5xl mx-auto glass border border-luxury-gold/20 p-5 md:p-6 flex flex-col md:flex-row items-start md:items-center gap-4 md:gap-8">
            ...
            <div className="flex-1">
              <p className="text-luxury-cream/80 text-xs font-inter font-light leading-relaxed">
                {l.cookieBannerText}{' '}
                <Link href={`/${locale}/gizlilik`} className="text-luxury-gold hover:underline transition-colors duration-200">
                  {l.cookiePrivacyLabel}
                </Link>{' '}
                {locale === 'tr' ? 've' : locale === 'de' ? 'und' : locale === 'ar' ? 'و' : locale === 'ru' ? 'и' : locale === 'fr' ? 'et' : 'and'}{' '}
                <Link href={`/${locale}/cerez-politikasi`} className="text-luxury-gold hover:underline transition-colors duration-200">
                  {l.cookiePolicyLabel}
                </Link>
                {locale === 'tr' ? ' hakkında daha fazla bilgi alabilirsiniz.' : locale === 'de' ? ' für weitere Informationen.' : locale === 'ar' ? '.' : locale === 'ru' ? '.' : locale === 'fr' ? ' pour en savoir plus.' : '.'}
              </p>
            </div>
